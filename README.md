...
² `hmi` bundles `ffmpeg` (prebuilt Ubuntu archive package, unpinned — floats with `ubuntu:24.04`)
and `fpga-isv` (the interactive-sim-viewer Python/Tk GUI, source-pinned via `FPGA_ISV_REV` and
...
Every `.def` (except `hmi`, single-stage — see ² above) is a two-stage
